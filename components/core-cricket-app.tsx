...
import { BroadcastDestinations } from "@/components/broadcast-destinations";\nimport { LiveReplayControls } from "@/components/live-replay-controls";
...
function LiveStudio({ authorized, canScore, canControlSponsors, sponsorCount, match, battingTeam, bowlingTeam, videoRef, cameraOn, broadcasting, watchLink, onCamera, onScoreDevice, onBroadcast, onSponsor }: { authorized: boolean; canScore: boolean; canControlSponsors: boolean; sponsorCount: number; match?: Match; battingTeam?: Team; bowlingTeam?: Team; videoRef: React.RefObject<HTMLVideoElement | null>; cameraOn: boolean; broadcasting: boolean; watchLink: string; onCamera: () => void; onScoreDevice: () => void; onBroadcast: () => void; onSponsor: () => void }) {
  ...
  return <div className="page"><SectionHead eyebrow="AUTHORIZED LIVE TOOLS" title="Live Studio" action={<div className="plan-status">{broadcasting ? "LIVE NOW" : "ACCESS INCLUDED"}</div>} /><BroadcastDestinations broadcasting={broadcasting} /><div className="studio-layout"><section className="camera-stage"><video ref={videoRef} playsInline muted /><div className={cameraOn ? "camera-empty hidden" : "camera-empty"}><div className="camera-icon">▣</div><h2>Broadcast from this phone</h2><p>Keep this device on the camera. A scorer can update the match from another phone.</p><button className="primary-button" onClick={onCamera}>Enable camera</button></div>{cameraOn && <div className="stream-overlay"><div><b>{battingTeam?.shortName}</b><strong>{match?.runs}/{match?.wickets}</strong><span>{overs(match?.balls ?? 0)} ov</span></div><i>vs</i><b>{bowlingTeam?.shortName}</b><em>CORE CRICKET</em></div>}{broadcasting && <div className="broadcast-badge"><i /> LIVE</div>}</section><aside className="studio-controls"><div className="card"><span>CAMERA</span><h3>{cameraOn ? "Camera ready" : "Connect your phone"}</h3><button className="secondary-button" onClick={onCamera}>{cameraOn ? "Restart camera" : "Test camera"}</button></div><div className="card"><span>LIVE SCORE</span><h3>{battingTeam?.shortName} {match?.runs}/{match?.wickets}</h3><p>Use a second phone, or score from this same device.</p>{canScore ? <button className="secondary-button" onClick={onScoreDevice}>Open scoring on this device</button> : <small>You can broadcast this match; only an official scorer or tournament creator can change the score.</small>}</div><div className="card sponsor-control-card"><span>SPONSOR OVERLAY</span><h3>Manual sponsor graphic</h3><p>{sponsorCount ? `${sponsorCount} sponsor${sponsorCount === 1 ? "" : "s"} ready. Each press shows the next sponsor for 8 seconds.` : match?.tournamentId ? "Add sponsor logos in the tournament first." : "Sponsor overlay controls stay available for friendly live broadcasts too."}</p><button className="secondary-button" disabled={!canControlSponsors || !broadcasting || sponsorCount === 0} onClick={onSponsor}>Show Sponsor</button><small>{!match?.tournamentId ? "No sponsor content is linked to this friendly match yet." : !canControlSponsors ? "Only the site admin or tournament organizer can trigger sponsor overlays." : !broadcasting ? "Start the live broadcast first." : sponsorCount === 0 ? "No tournament sponsors available." : "Press again for the next sponsor."}</small></div><LiveReplayControls matchId={match.id} broadcasting={broadcasting} cameraOn={cameraOn} videoRef={videoRef} /><div className="card broadcast-card"><span>LIVE BROADCAST</span><h3>{broadcasting ? "Your match is live" : "Create a public watch link"}</h3><button className={broadcasting ? "stop-button" : "primary-button"} onClick={onBroadcast}>{broadcasting ? "Stop live" : "Go live"}</button>{watchLink && <div className="watch-share"><input readOnly value={watchLink} /><button onClick={() => void navigator.clipboard.writeText(watchLink)}>Copy link</button><a href={`https://wa.me/?text=${encodeURIComponent(`Watch this match live on Core Cricket: ${watchLink}`)}`} target="_blank" rel="noreferrer">WhatsApp</a></div>}</div></aside></div></div>;
}
