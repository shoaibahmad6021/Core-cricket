  ...
  useEffect(() => {
    ...
    const updateStatus = () => {
      ...
      if (!recorder || recorder.state === "inactive") {
        status.textContent = "Start Live to build the replay buffer.";
        if (replayButton) replayButton.disabled = true;
        return;
      }
      const buffered = Math.max(0, Math.min(20, Math.floor((Date.now() - segmentStartedAt) / 1000)));
      status.textContent = buffered >= Math.min(selectedSeconds, 4)
    ...
    const buildRecorder = (stream: MediaStream) => {
      ...
      try {
        return mimeType
          ? new MediaRecorder(stream, { mimeType, videoBitsPerSecond: 2200000, audioBitsPerSecond: 96000 })
    ...
    const finalizePlayableClip = async () => {
      ...
      // Immediately begin a fresh rolling segment. The WebRTC live feed is unaffected.
      recorder = null;
      chunks = [];
    ...
    const triggerReplay = async () => {
      ...
      if (buffered < 3) {
        if (status) status.textContent = "Keep Live running for at least 3 seconds before replay.";
        return;
      }

      if (status) status.textContent = "Finalizing a playable replay clip…";
      const finalized = await finalizePlayableClip();
      if (!finalized?.blob.size) {
        if (status) status.textContent = "Unable to create replay clip on this phone.";
      ...
      try {
        ...
        if (status) status.textContent = `↻ REPLAY ON AIR · ${actualDuration}s · live feed continues in background`;
